...
void main() {
  ...
  group('Ebsi request credential', () {
    ...
    group('getIssuer', () {
      ...
      test(
          'get issuer with credentialRequestUri when PreAuthorizedCode is given',
          () {
        const expectedIssuer =
            'https://talao.co/sandbox/ebsi/issuer/vgvghylozl';
  ...
  group('get did', () {
    ...
    group('send Presentation', () {
      ...
      test('successfully send presentation', () async {
        const response =
            '{"access":"ok","created":1676566727.680238,"credential_status":"signature check bypassed","holder_did_status":"ok","id_token_status":"ok","qrcode_status":"ok","response_format":"ok","status_code":200,"vp_token_status":"ok"}';
        dioAdapter.onGet(
          url,
      ...
      test('throw exception', () async {
        expect(
          () async {
            ...
            await ebsi.sendPresentation(
                uri, credentialToBePresented, mnemonic, null);
          },
          throwsA(isA<Exception>()),
        );
      });
