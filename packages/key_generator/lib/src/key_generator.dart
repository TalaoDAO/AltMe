...
class KeyGenerator {
  Future<String> privateKey(String mnemonic) async {
    ...
    // remove "=" padding 43/44 // ad -> LP5JxiklWcG-aHT0vDIfgBllOYQxhGmZpiAn
    // 7ZFYP5o

    final mx = pub.substring(0, 64);
    // first 32 bytes // mx -> 41a72ac211e17bfa49bde064d4617ed88f08d394d
    // fdbf20f51c367035d0dc9e8

    final ax = HEX.decode(mx);
    //[65, 167, 42, 194, 17, 225, 123, 250, 73, 189, 224, 100, 212, 97, 126, 216
    // , 143, 8, 211, 148, 223, 219, 242, 15, 81, 195, 103, 3, 93, 13, 201, 232]

    final x = base64Url.encode(ax).substring(0, 43);
    // remove "=" padding 43/44 // x -> QacqwhHhe_pJveBk1GF-2I8I05Tf2_
    // IPUcNnA10Nyeg

    final my = pub.substring(64);
    // last 32 bytes // my -> 901f5ac8c57c64f9306b0ed200e08a1b36a08969dc17
    // 55f149eac0e3b652a06c

    final ay = HEX.decode(my);
    //[144, 31, 90, 200, 197, 124, 100, 249, 48, 107, 14, 210, 0, 224, 138, 27,
    //54, 160, 137, 105, 220, 23, 85, 241, 73, 234, 192, 227, 182, 82, 160, 108]
    ...
    // alg "ES256K-R" for did:ethr and did:tz2
