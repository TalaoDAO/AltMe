...
void main() {
  ...
  group('EBSI DID and JWK', () {
    ...
    test('JWK from mnemonic', () async {
      final jwk = await oidc4vc.privateKeyFromMnemonic(mnemonic: mnemonic);
      expect(jsonDecode(jwk), expectedJwk);
    });

    group('getPrivateKey', () {
      test('privateKey from mnemonic', () async {
        final jwk = await oidc4vc.getPrivateKey(mnemonic, null);
      ...
      test('privateKey from mnemonic', () async {
        ...
        final jwk = await oidc4vc.getPrivateKey(null, jsonEncode(key));
        expect(jwk, expectedJwk);
      });
    });
    ...
    test('isEbsiUrl is true', () {
      const openIdRequest =
          'openid://initiate_issuance?issuer=https%3A%2F%2Fapi.conformance.intoidc4vc.xyz%2Fconformance%2Fv2&credential_type=https%3A%2F%2Fapi.conformance.intoidc4vc.xyz%2Ftrusted-schemas-registry%2Fv2%2Fschemas%2FzCfNxx5dMBdf4yVcsWzj1anWRuXcxrXj1aogyfN1xSu8t&conformance=ec4d929a-5439-46fb-b7d3-4a1a80d0c199';

      final isCorrectUrl = oidc4vc.isEbsiInitiateIssuanceUrl(openIdRequest);
      expect(isCorrectUrl, true);
    });

    test('isEbsiUrl is false', () {
      final isCorrectUrl =
          oidc4vc.isEbsiInitiateIssuanceUrl('https://example.com');
  ...
  group('EBSI: getAuthorizationUriForIssuer', () {
    ...
    test('get correct issuer from openid request', () {
      ...
      final issuer = oidc4vc.getIssuerFromOpenidRequest(
        givenOpenIdRequest,
      );
    ...
    test('get correct Autorization RequestParameters', () {
      const parameters = {
        ...
        'authorization_details':
            '[{"type":"openid_credential","credential_type":"https://api.preprod.oidc4vc.eu/trusted-schemas-registry/v1/schemas/0xbf78fc08a7a9f28f5479f58dea269d3657f54f13ca37d380cd4e92237fb691dd","format":"jwt_vc"}]',
        'redirect_uri':
            'app.altme.io/app/download/callback?credential_type=https://api.preprod.oidc4vc.eu/trusted-schemas-registry/v1/schemas/0xbf78fc08a7a9f28f5479f58dea269d3657f54f13ca37d380cd4e92237fb691dd&issuer=https://talao.co/sandbox/ebsi/issuer/vgvghylozl',
        'state': null,
        'op_state': null
      };
      final oidc4vc = OIDC4VC(client);
  ...
  group('issuer', () {
    ...
    test('get issuer when openIdRequest is good', () {
      ...
      const expectedIssuer =
          'https://api.conformance.intoidc4vc.xyz/conformance/v2';

      final issuer = oidc4vc.getIssuerRequest(openIdRequest);
      expect(issuer, expectedIssuer);
    });
    ...
    test('get empty string ' ' when openIdRequest is good', () {
      ...
      final oidc4vc = OIDC4VC(client);
      expect(oidc4vc.getIssuerRequest(openidRequest), '');
    });
  });
