...
class SendReceiveHomeCubit extends Cubit<SendReceiveHomeState> {
  ...
  }) : super(SendReceiveHomeState(selectedToken: selectedToken));
  ...
  Future<void> init({String baseUrl = ''}) async {
    try {
      ...
      try {
        ...
        selectedToken = state.selectedToken.copyWith(
          balance: '0',
          tokenUSDPrice: 0,
          balanceInUSD: 0,
        );
      ...
      await getOperations(baseUrl: baseUrl);
    } catch (e, s) {
      getLogger(
        runtimeType.toString(),
      ).e('error in init() e: $e, $s', error: e, stackTrace: s);
      if (isClosed) return;
      emit(
        state.error(
  ...
  Future<void> getOperations({String baseUrl = ''}) async {
    try {
      ...
      getLogger(
        runtimeType.toString(),
      ).e('error in getOperations() e: $e, $s', error: e, stackTrace: s);
      emit(
        state.error(
          messageHandler: ResponseMessage(
            message: ResponseString
  ...
  Future<List<OperationModel>> _getFa2Transfers(String baseUrl) async {
    ...
    final operations = result
        ...
        .toList();
    operations.sort((a, b) => b.dateTime.compareTo(a.dateTime));
    return operations;
  }
            ...
            .map(
              (dynamic e) => OperationModel(
                ...
                sender: OperationAddressModel(
                  address: e['from_address'] as String,
                ),
                ...
                target: OperationAddressModel(
                  address: e['to_address'] as String,
                ),
